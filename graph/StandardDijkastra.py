"""
    Standard Heap optimized Dijkstra's algorithm for Single-source shortest path problem 
...
class Dijkstra:
    def __init__(self, V: int=0, s: int=0, edges: list[tuple[int, int, int]]=None) -> None:
        """O((V+E)logV) Standard Heap optimized Dijkstra's algorithm for Single-source shortest path problem"""
        ...
        # Standard Dijkstra's algorithm O((V+E)logV)
        # 1. start with a source vertex
        self.minHeap = [(0, s)]      # min-heap: store unsolved vertices, element is a tuple (dist[u], u), initialize as source vertex to be enqueued
        while self.minHeap: 
            # 2. out of unvisited vertices, select the vertex with the smallest tentative distance and 'settle' it
            d, u = heappop(self.minHeap)   # dequeue O(logV)
    ...
    def relax(self, u: int, v: int, w: int) -> None:
        """O(logV) Relaxation  
           ...
           enqueue AFTER updating the distance
        ...
        """
        # case 1: the path can't be shortened
        if self.dist[v] <= (d := self.dist[u] + w):
            return

        self.dist[v] = d                         # 'relax' the edge u -> v  
        self.parents[v] = u                       # update parent of vertex v 
        heappush(self.minHeap, (d, v))               # enqueue after updating the distance O(log V) 
